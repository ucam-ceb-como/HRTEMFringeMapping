function [out1,out2,out3] = ginput_custom(arg1, x0, y0)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig,x0,y0);
...
function initialState = setupFcn(fig,x0,y0)
...
[initialState.CrossHair circle] = createCrossHair(fig,x0,y0);
...
set(fig,'WindowButtonMotionFcn',@(o,e) updateCrossHair(o,initialState.CrossHair,x0,y0,circle));
...
function updateCrossHair(fig, crossHair,x0,y0,circle)
...
% axes_handle = gca;
% cp = get(axes_handle, 'CurrentPoint');
% cp = cp(1,1:2);
% y0 = figHeight-y0;
% figPos = size(fig);
% figHeight =1095.7513;
...
% centre = [x0, y0];
% [a b] = hgconvertunits(fig, [x0 y0], fig.Units, 'pixels', fig);
% [a b] = 0;
% radius = sqrt((a-cp(1))^2 + (b-cp(2))^2);
% circle.Position = [centre-radius radius/sqrt(2)*2 radius/sqrt(2)*2];
% set(circle, 'Position', [centre-radius radius/sqrt(2)*2 radius/sqrt(2)*2]);
% circle_x = a-radius; 
% circle_y = b+radius;
% set(circle, 'Position', [circle_x circle_y radius/sqrt(2)*2 radius/sqrt(2)*2]);
% circle.Position = [circle_x circle_y radius/sqrt(2)*2 radius/sqrt(2)*2];
% delete(circle)
d = 100;
axes_handle = gca;
pt = get(axes_handle, 'CurrentPoint');
...
set(circle,'Position',pos,'Curvature',[1 1]);


title(gca, ['x0 = ', num2str(x0), ...
    ' ,y0 = ', num2str(y0), ...
    ' ,cp(1) = ', num2str(cp(1)), ...
    ' ,cp(2) = ', num2str(cp(2)), ...
...
function [crossHair circle] = createCrossHair(fig, x0, y0)
...
pos = [c-1 2 2];
% pos = [2 2 100 100];
circle = rectangle('Position',pos,'Curvature',[1 1], 'EdgeColor','g','LineWidth',1);
dot = rectangle('Position',pos,'Curvature',[1 1], 'EdgeColor','r','LineWidth',1.5);
